    ...
    def test_down_topology_uses_arch_and_cu_count(self):
        ...
        with (
            ...
            unittest.mock.patch.object(
                torch.cuda,
                "is_available",
                side_effect=AssertionError("AOT target must not probe CUDA"),
            ),
        ):
            self.assertEqual(get_device_cache_key(), (None, "gfx942", "80"))

    def test_qwen_ptpc_whole_graph_aot_jobs(self):
        ...
        self.assertEqual(len(whole_graph_jobs), 34)
        self.assertEqual({job["cu_num"] for job in whole_graph_jobs}, {80})
        self.assertEqual({job["quant_type"] for job in whole_graph_jobs}, {"ptpc"})
