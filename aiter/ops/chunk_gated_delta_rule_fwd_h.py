...
def chunk_gated_delta_rule_fwd_h_hip_fn(
    ...
    initial_state/final_state: [N, H, V, K] in dense mode. When
    initial_state_indices is provided, initial_state is a state pool
    [pool_size, H, V, K] and final state is written back in place.
    ...
    if inplace:
        ...
        if state_dtype is not None and initial_state.dtype != state_dtype:
            raise ValueError(
                f"`initial_state.dtype` ({initial_state.dtype}) must match `state_dtype` ({state_dtype})."
            )
    ...
    if not output_final_state:
        ...
        final_state = torch.empty((N, H, V, K), device=k.device, dtype=resolved_state_dtype)

    state_indices = (
        initial_state_indices.to(torch.int32).contiguous()
        if has_indices
        else torch.empty(0, device=k.device, dtype=torch.int32)
    )
