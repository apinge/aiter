...
def chunk_gated_delta_rule_fwd_kernel_h_opt_vk(
    ...
    SAVE_NEW_VALUE: tl.constexpr,
    IS_VARLEN: tl.constexpr,
    POOL_INDEXED_STATE: tl.constexpr,
    ...
    stride_k = Hg * K
    i_state = i_n
    if POOL_INDEXED_STATE:
        i_state = tl.load(initial_state_indices + i_n).to(tl.int64)
    if USE_INITIAL_STATE:
        h0 = h0 + ((i_state * H + i_h) * V * K).to(tl.int64)
    if STORE_FINAL_STATE:
        ht = ht + ((i_state * H + i_h) * V * K).to(tl.int64)
...
def chunk_gated_delta_rule_fwd_h_opt_vk(
    ...
    w and u are expected in head-major contiguous layout [B, H, T, K] / [B, H, T, V].
    initial_state/final_state: [N, H, V, K]. When initial_state_indices is
    provided, initial_state is a state pool [pool_size, H, V, K] and the final
    state is written back to the indexed pool slots in place.
    ...
    if initial_state_indices is not None and initial_state_indices.shape[0] != N:
        raise ValueError(
            "`initial_state_indices` length must match the number of sequences, "
            f"got {initial_state_indices.shape[0]} and {N}."
        )
    state_indices = (
    ...
    pool_indexed_state = has_indices
    ...
    chunk_gated_delta_rule_fwd_kernel_h_opt_vk[grid](
        ...
        BT=BT,
        USE_EXP2=use_exp2,
        POOL_INDEXED_STATE=pool_indexed_state,
    )
    return h, v_new, final_state
