...
def get_device_cache_key():
    ...
    # AOT workers are forked after the parent has imported torch. When the
    # target is explicit, avoid touching the CUDA runtime in the child.
    if (
        os.environ.get("COMPILE_ONLY") == "1"
        and target_arch is not None
        and target_cu_count is not None
    ):
